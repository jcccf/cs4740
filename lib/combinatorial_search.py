    ...
    with open('data/output/searchresult.txt', 'w') as f:
        for pos_window_size in [0,1,5,10]:
            for ngram_size in [0,1,2]:
                for window_size in [0,1,2,3,5,10,20,40]: #don't think the contexts are large enough to need to go higher
                    for use_syntactic_features in [0,1]:
                        for use_lesk in [False,True]:
                            for use_lesk_words in [False,True]:
                                ...
                                print "%d %d %d %d %d %f %f %f"%(ngram_size,window_size,use_syntactic_features,use_lesk,use_lesk_words,prec,rec,f1)
                                f.write("%d %d %d %d %d %f %f %f"%(ngram_size,window_size,use_syntactic_features,use_lesk,use_lesk_words,prec,rec,f1))
